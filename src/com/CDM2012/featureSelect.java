...
public class featureSelect {
	public static void main(String[] args){
		...
		double[][] data = file.openfile();//把数据读成double型的二维数组
		
		double accuracy = 0;
		
		double[] cls1 = new double[data.length];//取时间序列
		...
		while(true){
			...
			for(int j = ql;j <= 50;j++){
				for(int i = 2;i <= c-ql;i++){
					...
					double[][] dis = CM.computeMatrix(ftr, data2); //-------------------------------输出正确
					...
					for( corr = 0.95;corr >= 0.65;corr=corr-0.01){
						...
						int[] ind1 = finds.FindS(dis,Math.sqrt(2*(1-corr)));//-------------有虚数
						FindL findl = new FindL();
						int[] ind2 = findl.FindL(dis, Math.sqrt(2*(1-corr)));//------------有虚数
